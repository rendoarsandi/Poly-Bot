...
// UpdatePrices updates the current prices
...
func (t *TUI) renderMarketInfo() string {
	...
	// Prices
	if len(t.outcomes) == 2 {
		p1 := t.lastPrices[t.outcomes[0]]
		p2 := t.lastPrices[t.outcomes[1]]
		sum := p1 + p2
		margin := (1.0 - sum) * 100
		...
		sb.WriteString(fmt.Sprintf("   💹 %s: $%.3f | %s: $%.3f\n",
			t.outcomes[0], p1, t.outcomes[1], p2))
		sb.WriteString(fmt.Sprintf("   📈 Sum: %.4f | %sMargin: %.2f%%%s\n",
