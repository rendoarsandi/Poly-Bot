...
func TestEvaluateReplayCountsModelAssistedSnapshotsAndTrades(t *testing.T) {
	now := time.Date(2026, 3, 9, 7, 0, 0, 0, time.UTC)
	report := EvaluateReplay(defaultFusionConfig(), []ReplaySnapshot{
		{
			Timestamp:            now,
			...
			UpAskDepth:           300,
			DownAskDepth:         300,
			Features:             ModelFeatures{FairUp: 0.66, Score: 0.08, Returns1m: 0.002, Returns5m: 0.003, TradeFlowImbalance: 0.3, OrderBookImbalanceL1: 0.2, ProbVelocity: 0.01, TrendRegime: 0.2, ExternalModelWeight: 0.30, ExternalModelReason: "xgb"},
		},
		{
			Timestamp:            now.Add(20 * time.Second),
			Asset:                "BTC",
	...
	if report.ModelSnapshots != 1 || report.ModelTrades != 1 {
		t.Fatalf("expected model usage counts, got %+v", report)
	}
	if report.AverageModelWeight != 0.30 {
		t.Fatalf("expected average model weight 0.30, got %+v", report)
	}
}
