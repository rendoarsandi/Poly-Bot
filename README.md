...
| **Time In Force (TIF)**| `IOC` | `cmd/realbot/main.go` | All orders use `TIFFillAndKill` instead of `FOK` (Fill-Or-Kill). This prevents Polymarket from rejecting the entire order if it's missing just a few shares. It takes exactly what is available and cancels the rest. |
