...
func run() error {
	...
	for {
		...
		if arbMode == paperArbModeCopytrade {
			...
			copytradeMarkets, marketsErr := paperbotFindCopytradeMarkets(ctx, restClient, copytradeTarget.Wallet, maxMarkets)
			if marketsErr != nil && len(copytradeMarkets) == 0 {
				logEvent(tui, csvLogger, engine, "WARN", "SYSTEM", "COPYTRADE_MARKETS", "Copytrade target market discovery failed: %v", marketsErr)
			}
